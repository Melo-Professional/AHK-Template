/************************************************************************
 * @description Modern Slider for AutoHotKey
 * @author Melo
 * @date 2026/07/15
 * @version 2.4.2 (Rounded Corners Update)
 ***********************************************************************/
/* 
; EXAMPLE
...
class ModernSlider {
    ...
    __New(guiObj, options := "", startValue := 0, min := 0, 
        ...
        this._value := startValue < min ?
            min : (startValue > max ? max : startValue)

        x := 10, y := 10, w := 200, h := 40 
        RegExMatch(options, "i)\bx(\d+)", &m) ?
            x := Integer(m[1]) : ""
        RegExMatch(options, "i)\by(\d+)", &m) ?
            y := Integer(m[1]) : ""
        RegExMatch(options, "i)\bw(\d+)", &m) ?
            w := Integer(m[1]) : ""
        RegExMatch(options, "i)\bh(\d+)", &m) ?
            h := Integer(m[1]) : ""
    ...
    RenderSliderBitmap() {
        ...
        ; Set rounded caps (LineCapRound := 2)
        ...
        ; --- 2. Draw Active Foreground Track (Rounded Line) ---
