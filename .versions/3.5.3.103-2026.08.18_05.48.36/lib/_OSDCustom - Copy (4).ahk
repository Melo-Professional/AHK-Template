 ...
 * @version 6.21.0 (Mouse Position and Rounded Corners fix)
 ***********************************************************************/
 ...
 */
    ...
    Show(Text?, Position := "", TimeOut := "", Progress := "") {
        ...
        if (OSDCustom.DWMCompatible && val >= 0 && val <= 3) {
            ...
            ; MODE 2: Custom Large Pixel Radius (e.g. 40, 60) + Injected CS_DROPSHADOW
            scaledRadius := OSDCustom.DPIScale(val)

            ; Apply Region Mask
            WinSetRegion("0-0 w" guiWidth " h" guiHeight " r" scaledRadius "-" scaledRadius, this.MyGui.Hwnd)

            ; Inject CS_DROPSHADOW Class Style
            GCL_STYLE := -26
            CS_DROPSHADOW := 0x00020000
            
            origStyle := DllCall(A_PtrSize = 8 ? "user32\GetClassLongPtr" : "user32\GetClassLong", "Ptr", this.MyGui.Hwnd, "Int", GCL_STYLE, "Ptr")
            DllCall(A_PtrSize = 8 ? "user32\SetClassLongPtr" : "user32\SetClassLong", "Ptr", this.MyGui.Hwnd, "Int", GCL_STYLE, "Ptr", origStyle | CS_DROPSHADOW, "Ptr")

            ; Force Win32 Frame Refresh
            DllCall("user32\SetWindowPos", "Ptr", this.MyGui.Hwnd, "Ptr", 0, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", 0x0027)

            if (origStyle !== 0)
                DllCall(A_PtrSize = 8 ? "user32\SetClassLongPtr" : "user32\SetClassLong", "Ptr", this.MyGui.Hwnd, "Int", GCL_STYLE, "Ptr", origStyle, "Ptr")
        ...
        if (StrLower(Position) == "mouse") {
            ...
            targetX := mX + DPIScale(56)
            targetY := mY + DPIScale(26)
            ;targetY := mY + DPIScale(32)
    ...
    AnimateSlideIn() {
        ...
        try {
			...
			if (reachedTarget) {
				...
				; --- SHADOW INJECTION FOR CUSTOM ROUNDED CORNERS ---
				; Must inject the class style BEFORE removing transparency,
				; otherwise DWM misses the transition and ignores the shadow.
				if (this.RoundedCorners > 3 && this.Opacity == 255) {
					GCL_STYLE := -26
					CS_DROPSHADOW := 0x00020000
					origStyle := DllCall(A_PtrSize = 8 ? "user32\GetClassLongPtr" : "user32\GetClassLong", "Ptr", hwnd, "Int", GCL_STYLE, "Ptr")
					DllCall(A_PtrSize = 8 ? "user32\SetClassLongPtr" : "user32\SetClassLong", "Ptr", hwnd, "Int", GCL_STYLE, "Ptr", origStyle | CS_DROPSHADOW, "Ptr")
					
					; Transition from Layered to Non-Layered triggers DWM frame recalculation
					WinSetTransparent("Off", hwnd)
					DllCall("user32\SetWindowPos", "Ptr", hwnd, "Ptr", 0, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", 0x0027)
					
					if (origStyle !== 0)
						DllCall(A_PtrSize = 8 ? "user32\SetClassLongPtr" : "user32\SetClassLong", "Ptr", hwnd, "Int", GCL_STYLE, "Ptr", origStyle, "Ptr")
				} else {
					; For translucent windows or native corners
					WinSetTransparent(this.Opacity == 255 ? "Off" : this.Opacity, hwnd)
				}
