 ...
 */
        ...
        ; OPTION 4 - SYSTEM ROUND, GOOD SHORT SHADOW
        ...
        try {
            ...
            NumPut("Int", 3, cornerPreference, 0) ; DWMWCP_ROUNDSMALL = 3 (Forces a tight, delicate shadow profile)
...
;@region log
/*
1.1.5 (OPTION 4 FOR SHORT CAST SHADOW, OPTION 5 FOR LONG CAST SHADOW, BOTH DONT USE CUSTOM ROUNDED CORNERS)

;OPTION 1 - long shadow
...
; OPTION 3 - GOOD, BUT SEEMS LIKE OPTION 5 AND 5 HAS  MORE OPTIONS
...
;OPTION 5 - GOOD ROUND, LONG SHADOW
; --- UNIVERSAL WINDOWS 11 BORDERLESS DROP SHADOW ENGINE (CLEAN ROUNDED CORNERS) ---
        try {
            ; 1. Force the Non-Client area rendering policy to "Enabled"
            ncPolicy := Buffer(4, 0)
            NumPut("Int", 2, ncPolicy, 0) ; DWMNCRP_ENABLED = 2
            ...
            ; 2. NATIVE DWM ROUNDING (Banish the White Corners!)
            ; This tells Windows 11 to handle the rounding natively, perfectly masking the background.
            cornerPreference := Buffer(4, 0)
            NumPut("Int", 2, cornerPreference, 0) ; DWMWCP_ROUND = 2 (Standard system rounded corner canvas)
            DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", this.MyGui.Hwnd, "UInt", 33, "Ptr", cornerPreference, "UInt", 4)

            ; 3. Inject a minimal hardware layout margin so the soft shadow map initiates smoothly
            margins := Buffer(16, 0)
            NumPut("Int", 1, margins, 0)  ; Left
            NumPut("Int", 1, margins, 4)  ; Right
            NumPut("Int", 1, margins, 8)  ; Top
