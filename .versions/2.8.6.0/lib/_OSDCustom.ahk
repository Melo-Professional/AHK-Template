...
class OSDCustom {
    ...
    Show(text := A_LineFile, position := "", duration := "") {
        if (position == "")
            position := OSDSettings.HasProp("Position") ? OSDSettings.Position : "x0.50 y0.50"
        if (duration == "") 
            duration := OSDSettings.TimeOut
        ...
        idealGuiWidth := textBounds.W + (OSDSettings.MarginX * 2)
        finalGuiWidth := Max(OSDSettings.MinWidth, Min(idealGuiWidth, OSDSettings.MaxWidth))
        finalTextWidth := finalGuiWidth - (OSDSettings.MarginX * 2)

        if (!this.MyGui) {
            this.MyGui := Gui(this.Options, this.Title)
            this.MyGui.OnEvent("Close", (*) => this.Destroy())
            
            this.MyGui.MarginX := OSDSettings.MarginX
            this.MyGui.MarginY := OSDSettings.MarginY
            
            this.MyGui.SetFont("s" OSDSettings.FontSize " w" OSDSettings.FontWeight, OSDSettings.FontName)
        ...
        this.MyGui.Show("w" finalGuiWidth " h" (textBounds.H + (OSDSettings.MarginY * 2)) " Hide")
        this.MyGui.GetPos(,, &guiWidth, &guiHeight)

    ; SHADOWS AND ROUNDED CORNERS
    if (VerCompare(A_OSVersion, "10.0.22000") >= 0) {
         ; --- UNIVERSAL WINDOWS 11 BORDERLESS DROP SHADOW ENGINE (TIGHT & DELICATE) ---
        try {
            ...
            NumPut("Int", 3, cornerPreference, 0) ; DWMWCP_ROUNDSMALL = 3
    ...
    ; --- FALLBACK FOR WINDOWS 10 (If Windows 11 technique wasn't applied) ---
    if (!shadowApplied) {
        ; WINDOWS 10 shadow
		; --- SAFELY ISOLATED WINDOWS 10 SHADOW (AHK v2) ---

		; Keep it clean: Remove standard window borders and caption
		;    this.MyGui.Opt("-Caption -Border")
		...
		;    (If your script shows the GUI elsewhere, make sure this section runs right before it becomes visible)
		DllCall("User32.dll\SetWindowPos", "Ptr", this.MyGui.Hwnd, "Ptr", 0, "Int", 0, "Int", 0, "Int", 0, "Int", 0, "UInt", 0x0047) ; SWP_SHOWWINDOW | SWP_FRAMECHANGED
