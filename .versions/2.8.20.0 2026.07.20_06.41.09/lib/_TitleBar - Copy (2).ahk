...
;        guiObj.OnEvent("Escape", (go) => this.TitleBars.Delete(go.Hwnd))
    ...
    static RenderButtons(tb) {
        ...
        if (cfg.Close) {
            btnX := "X" . (w - btnWidth)
            tb.Buttons["Close"] := guiObj.Add("Text", btnX . " Y0 W" btnWidth " H" btnHeight " +Center +0x200 +0x100 +Background000000", Chr(0xE8BB))
        ...
        if (cfg.Max) {
            ...
            tb.Buttons["Max"] := guiObj.Add("Text", btnX . " Y0 W" btnWidth " H" btnHeight " +Center +0x200 +0x100 +Background000000", Chr(0xE922))
            tb.Buttons["Max"].OnEvent("Click", (*) => WinGetMinMax(guiObj.Hwnd) ? guiObj.Restore() : guiObj.Maximize())
        }
        if (cfg.Min) {
            offset := (cfg.Close ? 1 : 0) + (cfg.Max ? 1 : 0) + 1
            btnX := "X" . (w - (btnWidth * offset))
            tb.Buttons["Min"] := guiObj.Add("Text", btnX . " Y0 W" btnWidth " H" btnHeight " +Center +0x200 +0x100 +Background000000", Chr(0xE921))
