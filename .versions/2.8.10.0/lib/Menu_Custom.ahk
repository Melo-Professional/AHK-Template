...
/*         ShowTrayNotification(
            "Welcome!", 
            AppName " is now active and running in your system tray."
            ) */
    ...
    ShowTrayNotification(title, message) {
        ...
        MyGui.SetFont("w700 s" Settings.GuiFontSizeBig, Settings.GuiFontName)
        TitleCtrl1 := MyGui.Add("Text", "w300 y0", title)
        TitleCtrl1.OnEvent("Click", CleanDestroy)
        MyGui.SetFont("w300 s" Settings.GuiFontSizeMedium, Settings.GuiFontName)
        MessageCtrl := MyGui.Add("Text", "w300 y+15", message)
        MessageCtrl.OnEvent("Click", CleanDestroy)
        MyGui.SetFont("w100 s" Settings.GuiFontSizeSmall, Settings.GuiFontName)
