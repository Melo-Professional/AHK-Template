/************************************************************************
 * @description Modern Slider for AutoHotKey v2 (Flawless Redraw & Zero Ghosting)
 * @author Melo
 * @date 2026/07/15
 * @version 2.4.1
 ***********************************************************************/
...
class ModernSlider {
    ...
    __New(guiObj, options := "", startValue := 0, min := 0, max := 100, callback := "", themeMode := "Auto", lightColors := "", darkColors := "") {
        ...
        ; Store the precise coordinates buffer with a 3px padding margin
        ; to fully capture and erase anti-aliased outer edges
        pad := 3
        ...
        ; Create standard 100% transparent Picture control
        this.sliderCtrl := guiObj.Add("Pic", "x" x " y" y " w" w " h" h " -Border -E0x200 +BackgroundTrans +0xE")
        this.hwnd := this.sliderCtrl.hwnd 
        
        ; Explicitly strip Win32 borders and styles to guarantee borderless rendering
        try {
    ...
    SetControlBitmap(hBitmap) {
        ...
        ; 1. Assign the fresh transparent GDI+ bitmap first using the cached HWND safely.
        hOld := SendMessage(0x172, 0, hBitmap, this.hwnd)
        if (hOld) {
            DllCall("gdi32\DeleteObject", "Ptr", hOld)
        }

        ; 2. Force the parent window to synchronously erase and repaint this exact coordinate space.
    ...
    RenderSliderBitmap() {
        ...
        trackY := (h / 2) - (this.trackH / 2)
        trackBgCol := (StrLen(colors[2]) == 8) ? Integer("0x" colors[2]) : (0xFF000000 | Integer("0x" colors[2]))
        
        DllCall("gdiplus\GdipCreateSolidFill", "Int", trackBgCol, "Ptr*", &pBrushBg:=0)
        DllCall("gdiplus\GdipFillRectangle", "Ptr", pGraphics, "Ptr", pBrushBg, "Float", this.paddingX, "Float", trackY, "Float", trackW, "Float", this.trackH)
        ...
        if (activeW > 0) {
            ...
            DllCall("gdiplus\GdipCreateSolidFill", "Int", trackActiveCol, "Ptr*", &pBrushActive:=0)
            DllCall("gdiplus\GdipFillRectangle", "Ptr", pGraphics, "Ptr", pBrushActive, "Float", this.paddingX, "Float", trackY, "Float", activeW, "Float", this.trackH)
            DllCall("gdiplus\GdipDeleteBrush", "Ptr", pBrushActive)
        }
