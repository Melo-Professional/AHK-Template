...
Backup() {
    ...
    Global backupMode, backupMinutesThreshold, backupSeparator, backupTemplateName, AppVersion, compressBackup

    AppVersion                  := IsSet(AppVersion)                ? AppVersion                : "0.0.0.0"              ; AppVersion
    backupMode                  := IsSet(backupMode)                ? backupMode                : "AppVersionOnly"       ; Options: "AppVersionOnly" or "AppVersionAndMinutes"
    backupMinutesThreshold      := IsSet(backupMinutesThreshold)    ? backupMinutesThreshold    : 30                     ; Minutes to check if backupMode is time-based
    backupTemplateName          := IsSet(backupTemplateName)        ? backupTemplateName        : "TemplateA"             ; Script name that triggers full lib folder copy
    backupSeparator             := IsSet(backupSeparator)           ? backupSeparator           : "-"                    ; Separator character for folder names
    timeFormat                  := IsSet(timeFormat)                ? timeFormat                : "yyyy.MM.dd_HH.mm.ss"  ; Time format
    compressBackup              := IsSet(compressBackup)            ? compressBackup            : false                  ; Toggles zipping backup
    ...
    ; Setup target paths
    ; -------------------------------------------------------------------
    timestamp := FormatTime(A_Now, timeFormat)
    ...
    ; -------------------------------------------------------------------
    ; 3. Handle compilation movement & final folder naming flags
    ; -------------------------------------------------------------------
    if (hasCompExe) {
        ...
        compiledTargetDir := targetDir backupSeparator "COMPILED"
        DirMove(targetDir, compiledTargetDir, 1)
        targetDir := compiledTargetDir
    }

    ; -------------------------------------------------------------------
    ; 4. Optional Backup Compression
    ; -------------------------------------------------------------------
    if (compressBackup) {
