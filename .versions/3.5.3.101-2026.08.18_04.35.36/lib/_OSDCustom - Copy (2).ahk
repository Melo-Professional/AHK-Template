 ...
 */
    ...
    Show(Text?, Position := "", TimeOut := "", Progress := "") {
        ...
        scaledRadius := OSDCustom.DPIScale(this.RoundedCorners)

        ; 1. Tell Windows 11 DWM not to override corners with system default rounding
        ; --- CLEAN ROUNDED CORNERS (SplashBanner Method) ---
        scaledRadius := OSDCustom.DPIScale(this.RoundedCorners)
        if (scaledRadius > 0) {
            ; Cuts the window region cleanly with zero background artifacts
            WinSetRegion("0-0 w" guiWidth " h" guiHeight " r" scaledRadius "-" scaledRadius, this.MyGui.Hwnd)
        } else {
            ; Value 0 = sharp 90-degree square corners
            WinSetRegion("", this.MyGui.Hwnd)
        }

        ; Optional: Apply Windows Class Drop Shadow (CS_DROPSHADOW)
        try {
            GCL_STYLE := -26
            ...
            ; Force OS to repaint frame with shadow immediately
        ...
        if (StrLower(Position) == "mouse") {
            ...
            targetX := mX + DPIScale(55)
            targetY := mY + DPIScale(25)
