...
class AutoUpdater {
    ...
    ; Automatically checks if the set frequency interval has passed
    CheckOnStartup() {
    ...
    CheckForUpdates(silent := false) {
        ...
        try {
            ...
            ; Extract latest tag name
            if RegExMatch(json, '"tag_name"\s*:\s*"([^"]+)"', &tagMatch)
                this.LatestVersion := tagMatch[1]

            ; Check for asset download link, fallback to zipball
            ...
            if (this.IsNewerVersion(this.App.Version, this.LatestVersion)) {
                if silent {
                    ; Prompt user softly on automatic detection
                    if (MsgBox("A new version (" . this.LatestVersion . ") is available for " . A_ScriptName . "!\n\nWould you like to update now?", "Update Available", "YesNo Iconi") == "Yes") {
                        this.ApplyUpdate()
                    }
                } else {
                    return true ; Used by GUI
                }
            } else if !silent {
            ...
            if !silent
                MsgBox("Failed to check for updates.\nError: " . err.Message, "Update Error", "48")
        }
        return false
    }

    ; Helper: Compare semantic versions (e.g. 1.0.0.0 vs 1.0.1.0)
    ...
    ; Performs download, replacement, and app restart
    ApplyUpdate() {
        if (this.DownloadUrl == "") {
        ...
        try {
            ...
            ToolTip()
            MsgBox("Failed to download update file.\n" . err.Message, "Download Failed", "48")
            return
        }

        ; Create a self-deleting command script to handle file swap post-exit
        cmdScript := A_Temp . "\ahk_updater_" . A_TickCount . ".cmd"
        cmdContent := '
        @echo off
        timeout /t 1 /nobreak > nul
        :wait
        tasklist /FI "PID eq ' . ProcessExist() . '" 2>nul | find /I "' . ProcessExist() . '" > nul
        if %ERRORLEVEL%==0 (
            timeout /t 1 /nobreak > nul
            goto wait
        )
        ...
        ; Run updater detached and terminate current process
        Run(A_ComSpec . " /c `"" . cmdScript . "`"", , "Hide")
        ExitApp()
    }

    ; GUI for managing auto-updates
    ShowGUI() {
        ...
        updGui.AddText("x10 h1 w280 0x10") ; Horizontal Line
        ...
        btnSave.OnEvent("Click", (*) => (
            this.App.UpdateAuto := Boolean(chkAuto.Value),
